...
class ReceiverNode(Node):
    def __init__(self, stop_event=None):
        ...
        self.current_left_arm_joints_deg = [0]*11
        self.current_right_arm_joints_deg = [0]*11
        ...
        self.robot_state_subscription = self.create_subscription(
            String,
            f'robot_state_topic_{DEVICE_ID}',
    ...
    def robot_action_callback(self, msg: String) -> None:
        try:
            ...
            print("[Receiver] JSON decode error:", e)
        except Exception as e:
            print("[Receiver] Error in robot_state_callback:", e)
    ...
    def robot_state_callback(self, msg: String) -> None:
        try:
            ...
            if len(joints) != 22:
                print(f"[RobotState] Warning: unexpected joint length {len(joints)}")
                return
        
            self.current_left_arm_joints_deg = joints[0:11]
            self.current_right_arm_joints_deg = joints[11:22]
            
            # print("[Receiver] Updated left arm joints:", self.current_left_arm_joints_deg)
            # print("[Receiver] Updated right arm joints:", self.current_right_arm_joints_deg)
        except json.JSONDecodeError as e:
            print("[Receiver] JSON decode error:", e)
        except Exception as e:
            print("[Receiver] Error in robot_state_callback:", e)
